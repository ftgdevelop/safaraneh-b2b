...
import { FlightReserveItemType } from "../../types/flights";
import Image from "next/image";
import DownloadPdfVoucher from "../booking/DownloadPdfVoucher";

type Props = {
  reserve: FlightReserveItemType;
};

const FlightReserveListItem: React.FC<Props> = (props) => {
  const { reserve } = props;

  const { id, status, username } = reserve;
  ...
  if (id && username) {
    linkToDetail = (
      <Link
        ...
        href={`/flight/reserveList/reserveDetail?username=${username}&reserveId=${id}`}
      >
        اطلاعات بیشتر
      </Link>
    );
  ...
  return (
    ...
    >
      <div className="lg:col-span-3 col-span-1 flex gap-3 items-center xs: pt-2">
        {reserve?.departureFlight.airline?.picture?.path && (
          <Image
            src={reserve.departureFlight.airline.picture.path}
            alt={reserve.departureFlight.airline.picture.altAttribute || ""}
            title={reserve.departureFlight.airline.name}
            width={60}
            height={60}
            className="w-10 h-10"
          />
        )}
        <h3 className="font-semibold"> {reserve.departureFlight.departureAirport.city.name || reserve.departureFlight.departureAirport.name || "--"}
          <ArrowLeft className="w-5 h-5 inline-block fill-neutral-500 mx-3" />
          {reserve.departureFlight.arrivalAirport?.city?.name || reserve.departureFlight.arrivalAirport?.name || "--"}
        </h3>
      </div>
      ...
      <div className="text-sm grid grid-cols-2 lg:col-span-2 col-span-1 gap-x-4">
        <div className="col-span-2 md:col-span-1 space-y-2">
          ...
          <div className="flex items-center md:justify-start justify-between w-full">
            <div className="flex items-center">
              <Calendar className={iconClassName} />
              <span className="ml-2 text-neutral-400">{reserve.tripType === "Return" ? "تاریخ رفت" : "تاریخ پرواز"}:</span>
            </div>
            <span className="text-left md:text-right md:w-auto">
              {dateDiplayFormat({
                date: reserve.departureTime,
                format: "dd mm yyyy",
                locale: "fa",
              })}
            </span>
...
export default FlightReserveListItem;
