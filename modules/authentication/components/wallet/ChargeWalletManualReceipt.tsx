...
import { createManualReceipt, getAllAccountNumbers } from '@/modules/payment/actions';
import Loading from '@/modules/shared/components/ui/Loading';
import { AccountNumbersType, CreateManualReceiptParameters, CurrencyType } from '@/modules/payment/types';
...
const ChargeWalletManualReceipt: React.FC = () => {
    ...
    const [selectedAccountNumberId, setSelectedAccountNumberId] = useState<string>("");
    ...
    const submitHandler = async (values: FormParams) => {
        ...
        if (response.status == 200) {
            ...
            dispatch(setAlertModal({
                type: "error",
                title: "خطا",
                message: response?.response?.data?.error?.message || "خطا در ثبت فیش واریزی!",
                isVisible: true
            }))
    ...
    const selectedAccountNumber = bankList?.find(x => x.id.toString() === selectedAccountNumberId);
                        ...
                        {bankList?.length ? (
                            <div>
                                <div className='flex items-center mt-12 mb-7 gap-5'>
                                    <h5 className='text-lg font-semibold whitespace-nowrap'> اطلاعات تراکنش </h5>
                                    <hr className='w-full' />
                                </div>
                                ...
                                <div className='mb-5'>
                                    ...
                                    <Select
                                        ...
                                        buttonClassName={errors.destinationBank && touched.destinationBank ? "border-red-500" : ""}
                                    />
                                    ...
                                    <Field
                                        ...
                                        validate={(value: string) => validateRequied(value, "لطفا حساب مقصد را انتخاب کنید")}
                                    />

                                    {errors.destinationBank && touched.destinationBank && <div className='text-red-500 text-xs'>{errors.destinationBank}</div>}
                                </div>

                                <div className='text-center'>
                                    {selectedAccountNumberPhoto}
                                </div>
                                ...
                                <div className='grid gap-x-5 grid-cols-1 sm:grid-cols-2'>
                                    <DatePicker
                                        ...
                                        validateFunction={(value: string) => validateRequied(value, "لطفا تاریخ واریز را انتخاب نمایید")}
                                        name='date'
                                        rtl
                                        onChange={e => {
                                    ...
                                    <TimePickerMobiscroll
                                        ...
                                        validateFunction={(value: string) => validateRequied(value, "لطفا ساعت واریز را انتخاب نمایید")}
                                    />
                                </div>
                                ...
                                <FormikField
                                    ...
                                    validateFunction={(value: string) => validateRequied(value, "لطفا شماره تراکنش را وارد کنید")}
                                    value={values.transactionNumber}
                                />

                                <div className='flex items-center my-10 gap-5'>
                                    <h5 className='text-lg font-semibold whitespace-nowrap'> اطلاعات حساب مبدا </h5>
                                    <hr className='w-full' />
                                </div>
                                ...
                                <FormikField
                                    ...
                                    validateFunction={(value: string) => validateRequied(value, "لطفا نام واریز کننده را وارد کنید")}
                                ...
                                <FormikField
                                    ...
                                    validateFunction={(value: string) => validateRequied(value, "لطفا نام بانک را وارد کنید")}
                                ...
                                <FormikField
                                    ...
                                    validateFunction={(value: string) => validateRequied(value, "لطفا شماره حساب را وارد کنید")}
