...
} from "@/modules/shared/helpers/validation";
...
const EditUserInformation: React.FC<Props> = (props) => {
  ...
  useEffect(() => {
    const fetchCountriesList = async () => {
      ...
      if (response?.data?.result?.items) {
        const sortedcountries = [...response.data.result.items].sort(
          (b: CountryItem, a: CountryItem) => {
  ...
  const initialValues: UpdateUserParameters = {
    ...
    isNewsLater:
      userData?.isNewsLater === undefined ? true : userData.isNewsLater,
    roleNames: [],
    ...
    isEmailConfirmed:
      userData?.isNewsLater === undefined ? false : userData.isEmailConfirmed,
    id: userData.id,
  };

  return (
    <div className="bg-white w-full border rounded-xl p-5 md:p-8 mb-5">
      <Formik
        validate={() => {
          return {};
        }}
      ...
      >
        {({
          ...
          return (
            <Form autoComplete="off" className="max-w-[520px]">
              <FormikField
                labelIsSimple
                showRequiredStar
                className="mb-5"
                ...
                validateFunction={(value: string) =>
                  validateEmail({
                    value: value,
                    reqiredMessage: "ایمیل کاربر را وارد نمایید!",
                    invalidMessage: "آدرس ایمیل معتبر نیست",
                  })
                }
              ...
              {!!countries && (
                <SelectWithSearch
                  ...
                  items={countries.map((item) => ({
                    label: item.name || item.nationality || "",
                    value: item.code || "",
                  }))}
                  validateFunction={(value: string) =>
                    validateRequied(value, "ملیت کاربر را انتخاب نمایید!")
                  }
