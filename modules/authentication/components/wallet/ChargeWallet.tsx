...
const ChargeWallet: React.FC = () => {
  ...
  const submitHandler = async (values: { amount: string }) => {
    ...
    const reserveId: string | undefined = pathArray
      ?.find((item) => item.includes("reserveId="))
      ?.split("reserveId=")[1];
    ...
    if (response.status == 200) {
      router.push(
        `https://${ServerAddress.Payment}/fa/User/Payment/PaymentRequest?tokenId=${response.data.result.tokenId}`,
      );
    } else {
      dispatch(
        setAlertModal({
          title: t("error"),
          message:
            response?.response?.data?.error?.message || "خطا در ارسال درخواست!",
          isVisible: true,
        }),
      );
      ...
      {({ errors, touched, setFieldValue, values }) => {
        return (
          <Form autoComplete="off" className="my-5">
            <div className="mb-5">
              <label className="text-sm mb-1">مبلغ افزایش شارژ (ریال)</label>
                ...
                <div className="flex gap-4 my-4">
                  {bankList.gateways.map((bank: any, index: number) => (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        setGatewayId(bank.id);
                      }}
                      className={`border px-10 py-3 text-sm text-center rounded-xl text-blue-700 select-none outline-none disabled:border-neutral-400 disabled:bg-neutral-200 disabled:grayscale ${
                        gatewayId === bank.id
                          ? "border-teal-500"
                          : "grayscale border-neutral-300"
                      }`}
                    >
                      <img
            ...
            {!!(router.query && router.query.status === "1") && (
              <div className="border border-neutral-300 rtl:border-r-2 rtl:border-r-blue-800 p-4 text-sm text-blue-800">
                <TikCircle className="w-6 h-6 fill-current inline-block " /> کیف
                پول شما با موفقیت شارژ شد
              </div>
            )}
